...
def enable_rules_engine(auth_cookies, handle):
    ...
    response = requests.put(f"{BASE_URL}/api/modules", json=update_data, cookies=auth_cookies)
    if response.status_code != 200:
        print(f"   Failed to enable rules_engine: {response.status_code} - {response.text}")
        return False
    ...
    if response.status_code == 200:
        ...
        if "rules_engine" in updated_config:
            ...
            if rules_engine.get("enabled"):
                ...
                print(f"   Number of rules: {len(rules)}")
                for rule in rules:
                    print(f"   Rule: {rule.get('name')} - {rule.get('condition')}")
                return True
            else:
                print("   Rules engine not enabled in response")
        else:
...
def test_auto_rejection(handle):
    ...
    if result.get("status") == "rejected":
        print(f"   ✅ Message auto-rejected as expected!")
        print(f"   Reason: {result.get('reason', 'No reason provided')}")
        return result.get("message_id")
    else:
        print(f"   ⚠️ Message not auto-rejected. Status: {result.get('status')}")
...
def get_reach_attempts(auth_cookies):
    ...
    response = requests.get(f"{BASE_URL}/api/reach/attempts", cookies=auth_cookies)
    if response.status_code != 200:
        print(
...
def test_manual_approve(auth_cookies, attempt_id):
    ...
    response = requests.post(
        f"{BASE_URL}/api/reach/attempts/{attempt_id}/approve", cookies=auth_cookies
    )
    if response.status_code != 200:
...
def test_manual_reject(auth_cookies, attempt_id, reason="Manual rejection test"):
    ...
    response = requests.post(
        f"{BASE_URL}/api/reach/attempts/{attempt_id}/reject",
        json=reject_data,
...
def test_block_sender(auth_cookies, attempt_id):
    ...
    response = requests.post(
        f"{BASE_URL}/api/reach/attempts/{attempt_id}/block", cookies=auth_cookies
    )
    if response.status_code != 200:
...
def main():
    ...
    try:
        ...
        # Test auto-rejection
        rejected_message_id = test_auto_rejection(handle)
        if not rejected_message_id:
        ...
        print(f"\nFound auto-rejected attempt: {auto_rejected_attempt.get('id')}")
        ...
        print("2. ✅ Face creation")
        print("3. ✅ Rules engine enabled and configured")
        print("4. ✅ Auto-rejection of 'quick sync' messages")
        print("5. ✅ Decision surface shows reach attempts")
        print("6. ✅ Manual approval works")
        print("7. ✅ Manual rejection works")
        print("8. ✅ Block sender works")
